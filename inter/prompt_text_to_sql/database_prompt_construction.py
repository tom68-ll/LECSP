...
DATA_PATH = "./prompt_text_to_sql/data_processed"
...
import jsonlines
import re
def extract_create_table_prompt_forwiki(prompt_db, db_id, limit_value=3, normalization=True):
    db_path = '../data/wikisql/tables.jsonl'
    table_now = {}
    with open(db_path,'r') as fp:
        for item in jsonlines.Reader(fp):
            if item['id'] == db_id:
                ...
                continue
    if not table_now:
        print("wrong dbid:",db_id)
        raise AssertionError("db id is Wrong")
...
def generate_db_prompt(dataset, db_id, prompt_db="CreateTableSelect", limit_value=3, normalization=True):
    if dataset != 'wikisql':
        ...
        if prompt_db.startswith("CreateTable"):
            if prompt_db.startswith("CreateTableSelectCol"):
                schema_prompt = extract_create_table_prompt_column_example(prompt_db, db_id, db_path, limit_value=limit_value, normalization=normalization)
            else:
                schema_prompt = extract_create_table_prompt_forwiki(prompt_db, db_id, limit_value=limit_value, normalization=normalization)
        else:
            print(prompt_db)
            raise NotImplementedError
