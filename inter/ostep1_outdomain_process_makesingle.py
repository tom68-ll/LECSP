...
if args.order == 0:
    input_path = "../data/inter_data/step0_out"
    output_path = '../data/inter_data/step1_out'
    data_cluster_path = "../data/cluster_result_order1"
    db_id_path = "../data/spider_task_stream"
else:
...
if __name__ == '__main__':
    ...
    get_text_root = "./prompt_forsingle/all_schema_of"
    get_label_root = "./info_for_step1/label_of"
    task_rate = [3,3,3,3,3,3,3]
    for task_now in range(7):
        if name_orders[task_now] == "spider":
            get_text = True
        else:
            get_text = False
        ...
        output_label = []
        if get_text:
            with open(get_text_root + str(task_now) + ".json" , 'r') as fp:
                text_infos = json.load(fp)
        else:
            text_infos = []
            ...
            if '***********************************************' in lines[i]:
                if start_toread == True:
                    ...
                    for nlq, sql in pairs[:task_rate[task_now]]:
                        ...
                        if get_text:
                            output_text.append(text_infos[sql_set_readed])
                        else:
                            output_text.append("")
                    sql_set_readed += 1
