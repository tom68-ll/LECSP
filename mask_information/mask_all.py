...
def read_in_all_data(data_path=DATA_PATH):
    ...
    training_data = json.load(open(os.path.join(data_path, "test.json")))
...
def get_all_question_query_pairs(data):
    ...
    for item in data:
        question_query_pairs.append((item['text'],item['example']['question_toks'], item['sql'], item['example']['db_id']))
...
def remove_all_from_clauses(query_keywords):
    ...
    while "FROM" in query_keywords:
        ...
        join_count = 1
        for keyword in query_keywords[from_location:min(end_token_locations)]:
            if keyword == 'JOIN':
                join_count += 1
        ...
        if join_count == 1:
            ...
            query_keywords = query_keywords[:from_location] + [FROM_SYMBOL.strip('}') + " JOIN " + str(join_count) + '}'] + query_keywords[min(end_token_locations):]
...
def strip_query(query, table):
    ...
    #query_tokenized = query.split(' ')
    cols_dict = {}
    for token in query_only_valued.split():
...
def process_question(question, values, nums, columns, values_dict,col_dict):
...
def process_question(question, values, nums, columns):
    ...
    question_lemed_stemed = [ps.stem(w) for w in question_lemed]
    replace_inds = [i for i, qt in enumerate(question_lemed_stemed) if qt in columns]
    print("question_lemed_stemed:{}".format(question_lemed_stemed))
    #print("question_stemed: {}".format(question_stemed))
    #print("replace_inds: {}".format(replace_inds))
    for ind in replace_inds:
        question_toks[ind] = COLUMN_SYMBOL
...
def main():
    ...
    for data_in,data_out in zip(data_paths,output_paths):
        ...
        for eid, (text,question, query, bd_id) in tqdm(enumerate(train_qq_pairs)):
            
            if question == ['List', 'the', 'first', 'and', 'last', 'name', 'of', 'all', 'players', 'who', 'are', 'left', '/', 'L', 'hand', 'in', 'the', 'order', 'of', 'birth', 'date', '.']:
                output_all.append({"question":question,"query":query,"output":[]})
                continue
            ...
            question_template,only_valued_question = process_question(question, values, nums, columns, values_dict,cols_dict)
            


            output_all.append({"text": text,
                               "sql":query,
                               "question_masked": question_template,
                               "sql_masked": pattern,
                               "example":{
                                   "db_id": bd_id,
                                   }
            ...
            output_all.append({"text": text,"question":question,"query":query,"output":output_data})
            '''
        
        if not os.path.exists(data_out):
            os.makedirs(data_out)
        dataf_out = os.path.join(data_out,'test.json')
